...
class Polishing(SingleArmEnv):
    ...
    def __init__(
        ...
        # parameter settings for the computing reward
        ...
        #reward mode is redundant as the other rewards have been removed. 
        self.reward_mode = self.task_config["reward_mode"]
        #for calculating the max possible reward
        ...
        # Caution for not letting completion reward to supersede learning of a sensible behaviour of the agent
    ...
    def reward(self, action=None):
        ...
        At each timestamp it begins initializing penalties and triggers to zero.
        The function then calculates the total force at the end effector of the robot and the goal position.
        
        Next, the function checks for collisions and joint limits. If either of these conditions are met, a penalty is applied to the reward.
        
        If there are no collisions or joint limit violations, the function checks if the robot is wiping.
        The function then calculates the dense reward as we set the reward_shaping by default as true.
        
        The function then calculates  reward components which include penalties for force deviations and rewards for maintaining contact and velocity within a certain range.
        
        The function also penalizes the distance from the goal position and the agent's perpendicular deviations from the path it needs to traverse.
        
        If the 7th via-point of our FlatObject is wiped (the traversal length amounts to slightly more than 30 cm), a task completion reward is added. 
        Finally, the function applies a penalty for large end effector accelerations, prints the results if required, scaling factors are 1 so it returns the reward as is.

        The calculation of the reward multipliers takes care of the reward ranging between [-2,2]
        ...
        #defined separately for logging purposes
        ...
        if self.check_contact(self.robots[0].robot_model):
            ...
            if self.sim.data.ncon != 0:
                ...
                for marker in self.objs[0].sites[:-1]:
                    ...
                    # for now, the distance 0.02 is hardcoded to be less than the radius of the eef
                    if v_dist < 0.02:                            
            ...
            if self.reward_shaping:
                ...
                '''
                Reward the agent for maintaining a contact and velocity within a certain range
                Velocity condition is added with the observation of choppy y movement on the real robot
                '''
                if self.sim.data.ncon > 1 and self.min_vel<self.robots[0]._hand_vel[1]<self.max_vel:
                    # Penalty for force deviation from target force
                    ...
                    '''
                    Penalties for force deviations are ultimately symmetric but we started off with different penalties
                    with the idea that the agent should be more penalized for excessive force than for low force
                    '''
                    ...
                    if self.total_force_ee >= self.safe_force_high:
                        ...
                        reward = reward - self.force_penalty 
                        if self.total_force_ee>=self.f_cap:
                            self.f_excess = 1

                    # Penalty for low force with the end-effector
                ...
                #Penalizing the distance from the goal position to motivate the agent to move towards the goal
                ...
                #Penalizing the agent's perpendicular deviations from the path it needs to traverse
                
                self.x_dist = np.linalg.norm(self._eef_xpos[0] - goal_pos[0])
                reward-= self.position_track_multiplier*self.x_dist

                # We stop at site 7 which corresponds to little over 30 cm from the first via-point                  
                ...
                '''
                We don't use ee_accel_penalty in sim but we added penalty to large accelerations seeing 
                the stick-slip phenomenon on the real robot
                ''' 
    ...
    def _load_model(self):
        ...
        # define objects to be wiped for the sampler. We only use Flat_top for now
        self.objs = []
        obj_names = ("Flat_top", "Incline")
        
        '''
        Create default (SequentialCompositeSampler) sampler by defining the x and y coordinate range for each object
        This may not be necessary if in a single object mode but it is added for the purpose of generality
        '''  
    ...
    def _setup_observables(self):
        ...
        '''
        Force sensor reading in the observation is measures the total force exerted by the end-effector in that instant
        but cache of past readings can also be used, as done in the real-robot. 
        '''        
    ...
    def _check_success(self):
        """
        Checks if Task succeeds i.e the agent has received the task completion reward
    ...
    def _post_action(self, action):
        ...
        Returns:
            3-tuple:
                ...
                - (dict) info about current env step which includes all the measurements that we want to log with our RL runs
        """
